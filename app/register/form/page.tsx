...
export default function RegisterFormPage() {
    ...
    const fetchCourses = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const flattenedCourses = Array.isArray(data) 
            ? data 
            : Object.values(data).flat() as Course[];
  ...
  const groupedCourses = courses.reduce((acc, course) => {
    const grupo = course.grupo_repense;
    if (!acc[grupo]) {
      acc[grupo] = [];
    }
    acc[grupo].push(course);
    return acc;
  }, {} as Record<GrupoRepense, Course[]>);
  ...
  const handleNext = async () => {
    if (step === 1) {
      ...
      if (isValid) {
        const selectedCourse = courses.find((c) => c.id === watchedValues.course_id);
